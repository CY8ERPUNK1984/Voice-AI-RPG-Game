...
describe('StoryService', () => {
  ...
  describe('getStoriesByGenre', () => {
    ...
    it('should return empty array for genre with no stories', async () => {
      // All genres are covered in our test data, so this tests the logic
      const stories = await storyService.getStoriesByGenre('adventure');
      expect(stories).toHaveLength(1);
      expect(stories[0].genre).toBe('adventure');
    });

    it('should return multiple stories for genres with multiple entries', async () => {
      // Add a test by checking that we get the right count for each genre
